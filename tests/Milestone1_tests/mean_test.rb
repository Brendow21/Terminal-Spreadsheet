require_relative '../../evaluator'
require_relative '../../interfaces/Expression'
require_relative '../../grid'
require_relative '../../runtime'
require_relative '../../serializer'
require_relative '../../interfaces/visitor'
require_relative '../../abstractions/arithmetic'
require_relative '../../abstractions/primitives'
require_relative '../../abstractions/bitwise'
require_relative '../../abstractions/casting'
require_relative '../../abstractions/cellLvalue'
require_relative '../../abstractions/cellRvalue'
require_relative '../../abstractions/logical'
require_relative '../../abstractions/relational'
require_relative '../../abstractions/statistical'

evaluator = Evaluator.new
serializer = Serializer.new

# Create Test Grid
grid = Grid.new(6,6)
runtime = Runtime.new(grid)
address_one = CellLvalue.new(IntegerPrimitive.new(0),IntegerPrimitive.new(0))
expression_one = Add.new(IntegerPrimitive.new(5), IntegerPrimitive.new(5))
grid.set_cell_expression(address_one, expression_one)

address_two = CellLvalue.new(IntegerPrimitive.new(3),IntegerPrimitive.new(2))
expression_two = Add.new(IntegerPrimitive.new(1), IntegerPrimitive.new(1))
grid.set_cell_expression(address_two, expression_two)

# Create Cells for ranges
cell1 = CellLvalue.new(IntegerPrimitive.new(1),IntegerPrimitive.new(2))
cell2 = CellLvalue.new(IntegerPrimitive.new(5),IntegerPrimitive.new(3))
grid.set_cell_expression(cell1, IntegerPrimitive.new(2))
grid.set_cell_expression(cell2, IntegerPrimitive.new(4))

grid.print_grid
puts 

mean = Mean.new(
  cell1, cell2
)
puts mean.traverse(serializer, runtime)
puts mean.traverse(evaluator, runtime).value
